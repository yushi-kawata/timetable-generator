/* ============================================================================
   教室表示（独立ページ）の中身 ── 台帳 A4-119 ／ C案（2026-09-24）
   ============================================================================
...
(function (root) {
  ...
  /** 何も触らずにこの時間が過ぎたら画面を伏せる（★社長決裁・当面そのまま） */
  ...
  /** 窓口の名前。★裏側（GAS）の CLASSROOM_ACTION_ と合わせること */
  ...
  function start() {
    ...
    // ★合い言葉が URL に残り続けるのを避ける。
    //   history.replaceState はサーバーに何も送らず、画面も動きません。
    //   ★bookmark には既に控えてあるので、消しても動き続けます。
    //   ★ブックマーク自体は残るので、次に開いたときも同じように動きます。
    if (bookmark.ok && window.history && window.history.replaceState) {
      try {
        window.history.replaceState(null, '', window.location.pathname);
      } catch (err) {
        // 消せなくても動く。消せないことを理由に止めない
      }
    }
