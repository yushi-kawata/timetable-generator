// ============================================================================
// 時間割ツール 裏側プログラム ─ 番人入り（2026-09-08）／台帳 A4-41
// ============================================================================
//
// ★★ 気をつけること（先に読んでください）★★
//
...
function doPost(e) {
  try {
    ...
    if (body.action === 'authStudent') {
      ...
      const data = sheet.getDataRange().getValues();
      if (data.length <= 1) {
        return json_({ ok: false, msg: 'no students' });
      }
      const headers = data[0];
      for (let i = 1; i < data.length; i++) {
        const obj = {};
        ...
        if (dxEmail === email && dxPw === password) {
          ['月','火','水','木','金'].forEach(d => { obj[d] = obj[d] === true || obj[d] === '○' || obj[d] === 'TRUE'; });
          audit_(authEmail, 'authStudent', true, '');
          return json_({
            ok: true,
            student: {
              name: obj.name || '',
      ...
      // ★どのメールで失敗したかは書かない（監査シートに生徒のメールを溜めないため）
      audit_(authEmail, 'authStudent', false, '');
      return json_({ ok: false, msg: 'auth failed' });
    }
    ...
    if (body.action === 'dxCheckIn') {
      ...
      if (!creds) {
        return json_({ ok: false, msg: 'student not found' });
      }
      try {
