// ============================================
// 時間割ツール 裏側プログラム ─ 修正版（2026-09-02）／台帳 A4-41
// ============================================
// 元 = 公開リポジトリ origin/main の gas-script.js（＝本番が使っている実物）
//
// この版で直したこと（2点。★必ず両方まとめて貼ること）
//   1. getStudents が dx_password（younetDXの平文パスワード）を返さなくなった
//   2. saveStudents が「空で届いたパスワードで既存を消す」のを止めた
//
// ★★まだ直っていないこと（これだけでは安全になりません）
//   ・入口の認証は入っていません。getStudents は今も氏名・学年・dx_email を
//     誰にでも返し、saveStudents は sheet.clear() で名簿ごと消せます。
//   ・このツールは公開サイト（GitHub Pages・PUBLICリポジトリ）から呼ばれるため、
//     合言葉をプログラムに埋めても一緒に公開されます＝秘密になりません。
//     既存の番人（資産台帳・リンク集）はポータルのログイン証明を検証する方式ですが、
//     このツールにはログインの仕組みがないため、そのままは載りません。
//   → 「このツールをどこに置くか」を決めないと、入口は塞げません（台帳 A4-45）。
//
// ★貼る順番：GASを先、画面を後。
//   旧GASのまま新しい画面を配ると、空のパスワードで上書きされて全員分が消えます。
//   新GAS + 旧画面 は安全です（空で届いても据え置くため）。
// ============================================
      ...
      if (prev.length > 1) {
        ...
        if (iPw >= 0) {
          for (let i = 1; i < prev.length; i++) {
            const pw = (prev[i][iPw] || '').toString();
            if (!pw) continue;
            const key = iEmail >= 0 && prev[i][iEmail]
              ? 'e:' + prev[i][iEmail].toString().trim().toLowerCase()
              : (iName >= 0 ? 'n:' + (prev[i][iName] || '').toString().trim() : '');
